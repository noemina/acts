...
class InternalSeed {
 ...
 public:
  InternalSeed(const InternalSpacePoint<SpacePoint>& s0,
               ...
               const InternalSpacePoint<SpacePoint>& s2, float z, bool nQualitySeeds);
  InternalSeed& operator=(const InternalSeed& seed);

  const std::array<const InternalSpacePoint<SpacePoint>*, 3> sp;
  float z() const { return m_z; }
	float nQualitySeeds() const { return m_nQualitySeeds; }

 protected:
  float m_z;
	float m_nQualitySeeds;
};
...
inline InternalSeed<SpacePoint>& InternalSeed<SpacePoint>::operator=(
  ...
  m_z = seed.m_z;
  sp = seed.sp;
	m_nQualitySeeds = seed.m_nQualitySeeds;
  return (*this);
}
